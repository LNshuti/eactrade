...
dfeac_productname_partner_id <- 
  ...
  summarise(imports = sum(import_value, na.rm = TRUE)/1000, 
            exports = sum(export_value, na.rm = TRUE)/1000) %>%
...
eac_imports_bp <-  
  dfeac_productname_partner_id %>%
  select(location_product, partner_year, imports) %>%
...
eac_exports_bp <-  
  dfeac_productname_partner_id %>%
   select(location_product, partner_year, exports) %>%
  pivot_wider(id_cols = location_product,
              names_from = partner_year, 
...
```{r}
ggraph(nodes_eac_products, layout = 'fr', weights = "weight") + 
  geom_edge_link() + 
  geom_node_point()
```
