...
countries_codes <- 
  read_csv_arrow(paste0(repo_, "/processed/rwa_sum_features.csv"), 
                as_data_frame = FALSE)
...
net_bp <- 
  ...
  as_tbl_graph() %>%
  activate(nodes) %>% 
  left_join(eac_tradebal_df,by = c("name" = "product_origin")) 

unweighted_graph <- 
  ggraph(net_bp, layout='lgl') +
  # geom_edge_link(data = net_bp, 
  #                aes(width = weight,alpha=weight),show.legend = FALSE) + 
  geom_node_point(aes(colour=to), cex = 6) +
  geom_node_text(aes(label = to),cex = 3) +
  #remove_all_axes  + 
